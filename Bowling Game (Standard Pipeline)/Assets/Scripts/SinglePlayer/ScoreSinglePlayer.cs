...
public class ScoreSinglePlayer : MonoBehaviour {
	...
	public void ScoreDisplay(int[] roundScores){
		totalPins = FindObjectOfType<Pokeball>().GetComponent<Pokeball>().pins.Length;
		...
		for (int i = 0; i < roundScores.Length; i++) {
			...
			if (roundScores [i] == -1) {
				scores_text [i].text = " ";
			}else if (roundScores [i] == totalPins) {
				scores_text [i].text = "X";
				roundScores [i + 1] = 0;
			}else if (roundScores [i] == 0) {
				scores_text [i].text = "-";
			}else if (i>0){
				if ((roundScores [i] + roundScores [i - 1] == totalPins) && (roundScores [i - 1]==totalPins)){

					scores_text [i].text = " ";
				}else if (roundScores [i] + roundScores [i - 1] == totalPins){
		...
		for (int i = 0; i < roundScores.Length - 1; i++) {
			...
			if (i % 2 == 0) {
				if ((i > 5) && (round > 5)) {
					...
					if ((roundScores [i - 2] == totalPins) && (roundScores [i - 4] == totalPins) && (roundScores [i - 6] == totalPins)) {              //3 strikes in a row
					
						total_scores [j - 1] = totalPins + sum1;
					...
					} else if ((roundScores [i - 2] == totalPins) & (roundScores [i - 4] == totalPins)) {									//2 strikes in a row
					...
					} else if (roundScores [i - 2] == totalPins) {																//1 strike
					...
					if ((roundScores [i - 2] == totalPins) & (roundScores [i - 4] == totalPins)) {										//2 strikes in a row
				...
				if ((i > 1) && (round > 1)) {	
					if ((roundScores [i - 2] + roundScores [i - 1] == totalPins) && (roundScores [i - 2] != totalPins)) {		//spare (and not strike)
